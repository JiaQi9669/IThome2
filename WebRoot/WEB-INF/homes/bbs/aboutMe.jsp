...
<body>
	...
	<div id="main">
		...
		<h3 style="padding-left: 20px;">
			<s:property value="#friend.realName" />
			参与评论过的问题：
		</h3>
		<center>
			...
			<s:iterator value="pageBean.recordList" id="po" var="rep" status="indexs">
						<tr>
							<td>
								${indexs.index+1 }
							</td>
							<td style="text-align: left;">
								<a href="postviewpost?id=${post.id }">
									<s:property value="%{#rep.post.name }" />
								</a>
							</td>
							<td>
								<s:property value="%{#rep.post.friend.pickName }" />
							</td>
							<%-- ID:<s:property value="#reply.id"/> --%>
							<td>
								<s:property value="%{#rep.post.count }" />
							</td>
							<td>
								${rep.post.dates }
							</td>
			...
			<s:iterator begin="pageBean.beginPageIndex"
				...
				<li class="paginItem ${pageBean.currentPage==pageNums?'current':''}">
					...
					<c:if test="${pageBean.currentPage != pageNums }">
						<a href="postabout?pageNum=${pageNums }">${pageNums }</a>
					</c:if>
				</li>
			</s:iterator>
		</ul>
