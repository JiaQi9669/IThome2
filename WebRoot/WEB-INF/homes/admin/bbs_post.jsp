...
<body>
	...
	<div class="result-wrap">
		<form name="myform" id="myform" action="postfindByKeyWord?id=${bordid }"
			method="post">
			<div class="result-title">
				<div class="result-list">
					<a id="updateOrd" href="javascript:void(0)"><i
						class="icon-font"></i>查询贴子</a> <input class="common-text"
						placeholder="关键字" name="name" type="text"> <input
		...
		<div class="result-content">
			<table class="result-tab" width="100%">
				<tr>
					<th colspan="10" style="text-align: center;">贴子总览</th>
				</tr>
				<tr>
					<th>编号</th>
					<th>名称</th>
				...
				<s:iterator value="pageBean.recordList" status="index">
					<tr>
						...
						<td>${reply.size() }</td>
						<td>${count }</td>
						<td><%@taglib uri="http://java.sun.com/jsp/jstl/core"
								prefix="c"%> <c:url var="url"
								value="postview">
								<c:param name="id" value="${id }"></c:param>
							</c:url> <a href="${url }">浏览该贴</a></td>

						<td><c:url var="url" value="postdel">
								<c:param name="id" value="${id }"></c:param>
							</c:url> <a href="${url }">删除</a></td>
					</tr>
				</s:iterator>
			</table>
			<div class="pagin">
						...
						<li
							...
							<c:if test="${pageBean.currentPage == pageNums }">
								...
								<c:if test="${searchType == 1 }">
									<a href="postfindByKeyWord?id=${bordid }&name=${searchName }&pageNum=${pageNums }">${pageNums }</a>
								</c:if>
								<c:if test="${searchType != 1 }">
									<a href="postlistbyborad?id=${bordid }&pageNum=${pageNums }">${pageNums }</a>
								</c:if>
							</c:if>
						</li>
					</s:iterator>
