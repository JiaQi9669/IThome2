				...
				<div>
					<form action="projectSelect" method="post">
						<table class="search-tab">
							<tr>
								<th width="70">关键字:</th>
								<td><input class="common-text" placeholder="项目名的关键字"
						...
						<table class="result-tab" width="100%">
							...
							<s:iterator value="pageBean.recordList">
								<tr>
									...
									<td ><a href="projectintroduce?id=${id }" style="color:blue;">${name }</a> <!--项目名称--></td>
									<td>${message }</td>
									<td>${packNum }</td>
									<td>${viewCounts }</td>
									<td>${dates }</td>
				...
				<li class="paginItem ${pageBean.currentPage==pageNums?'current':''}">
					...
					<c:if test="${pageBean.currentPage != pageNums }">
						<c:if test="${projecttype != 1 }">
							<a href="projectsubl?pageNum=${pageNums }">${pageNums }</a>
						</c:if>
						<c:if test="${projecttype == 1 }">
							<a href="projectSelect?name=${projectnames }&pageNum=${pageNums }">${pageNums }</a>
						</c:if>
					</c:if>
				</li>
			</s:iterator>
